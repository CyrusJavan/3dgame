...
public:
    void set(const ofVec3f &f) { currentForce = f; }
    ThrusterForce(const ofVec3f & force);
    void updateForce(Particle *);
    void add(const ofVec3f&);
};

class DirectionalImpulseForce: public ParticleForce {
    ofVec3f currentForce;
public:
    void set(const ofVec3f &f) { currentForce = f; }
    DirectionalImpulseForce(const ofVec3f & force);
    void updateForce(Particle *);
};
