...
//
ThrusterForce::ThrusterForce(const ofVec3f &force) {
    currentForce = force;
}

void ThrusterForce::add(const ofVec3f &force){
    currentForce += force;
}
...
void ThrusterForce::updateForce(Particle * particle) {
    ...
    // f = mg
    //
    particle->forces += currentForce;
}

DirectionalImpulseForce::DirectionalImpulseForce(const ofVec3f &force) {
    currentForce = force;
    applyOnce = true;
}

void DirectionalImpulseForce::updateForce(Particle * particle) {
    //
    // f = mg
    //
    particle->forces += currentForce;
